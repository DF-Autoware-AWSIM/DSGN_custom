...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  //THCState *state = at::globalContext().lazyInitCUDA(); // TODO replace with getTHCState //Arka
  ...
  cudaMalloc((void**)&mask_dev, boxes_num * col_blocks * sizeof(unsigned long long));//Arka
...
if (err != cudaSuccess) {
    ...
    auto options = at::TensorOptions().device(boxes.device()).dtype(at::kLong);
    return at::empty({0}, options);  // return an empty tensor
}//Arka
